...
import {IVault} from "src/deprecated/v1/core/Vault.sol";
import {IConfig} from "src/deprecated/v1/core/Config.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import {SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import {IPosition} from "src/deprecated/v1/position/IPosition.sol";
...
contract MultiAssetPosition is IPosition, ERC721, Ownable2Step, ReentrancyGuard {
    ...
    uint256 public constant INITIALIZATION_THRESHOLD = 20000;
    uint256 public constant LIQUIDATION_THRESHOLD = 15000;
    uint256 public maxLiquidationBonusRate = 11500;
    address public liquidator;

    constructor(address _config, address _oracle) Ownable(msg.sender) ERC721("Position", "POSITION") {
        config = _config;
        oracle = _oracle;
    ...
    function setMaxLiquidationBonusRate(uint256 _maxLiquidationBonusRate) external onlyOwner {
        require(_maxLiquidationBonusRate >= 10500 && _maxLiquidationBonusRate <= 11500, "Invalid bonus rate");
    ...
    function supply(uint256 _tokenId, address _vToken)
        external
        nonReentrant
        onlyVault(_vToken)
        balanceCheck(_tokenId, _vToken, BalanceType.NO_DEBT)
    {
        int256 amount = int256(IERC20(_vToken).balanceOf(address(this)) - reserves[_vToken]);
    ...
    function withdraw(uint256 _tokenId, address _vToken, uint256 _amount) external nonReentrant onlyOwnerOf(_tokenId) {
        _withdraw(_tokenId, _vToken, _amount);
    }

    function _withdraw(uint256 _tokenId, address _vToken, uint256 _amount)
        private
        onlyVault(_vToken)
    ...
    function borrow(uint256 _tokenId, address _vToken, uint256 _amount)
        external
        nonReentrant
        onlyVault(_vToken)
    ...
    function repay(uint256 _tokenId, address _vToken, uint256 _amount) external nonReentrant onlyOwnerOf(_tokenId) {
        require(_amount > 0, "Amount must be greater than 0");
        _claim(_vToken);
        _repay(_tokenId, _vToken, _amount, msg.sender);
    }

    function _repay(uint256 _tokenId, address _vToken, uint256 _amount, address _payer)
        private
        onlyVault(_vToken)
    ...
    function _updateBalance(uint256 _tokenId, address _vToken, int256 _amount) private {
    ...
    function _claim(address _vToken) private {
        uint256 diff = IERC20(_vToken).balanceOf(address(this)) - reserves[_vToken];
        IERC20(_vToken).safeTransfer(msg.sender, diff);
    }

    function getBalance(uint256 _tokenId, address _vToken) public view returns (int256) {
        return balances[_tokenId][_vToken];
    }

    function getPosition(uint256 _tokenId) public view returns (address[] memory _vTokens, int256[] memory _balances) {
        _vTokens = new address[](positions[_tokenId].vaults.length());
        _balances = new int256[](positions[_tokenId].vaults.length());
    ...
    function health(uint256 _tokenId) public view returns (uint256 collateral, uint256 debt) {
        address[] memory _vTokens = positions[_tokenId].vaults.values();
        for (uint256 i = 0; i < _vTokens.length; i++) {
            uint256 absBalance = balances[_tokenId][_vTokens[i]].abs();
    ...
    function liquidate(uint256 _tokenId, bytes memory _data) external onlyLiquidator {
        ...
        // For vault holders
        require(debtAfter * collateralBefore < debtBefore * collateralAfter, "Health decreased");

        // For position holders and vault holders
        require(rewardValue <= (repaidValue * maxLiquidationBonusRate) / 10000, "Too much reward");
        return;
    }
}
