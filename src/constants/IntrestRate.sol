...
library IntrestRate {
    ...
    function calculateInterest(uint principal, uint interestRatePerSecond, uint duration) internal pure returns (uint) {
        return calculatePrincipalPlusInterest(principal, interestRatePerSecond, duration) - principal;
    }

    function calculatePrincipalPlusInterest(uint principal, uint interestRatePerSecond, uint duration) internal pure returns (uint) {
        return principal * rpow(
            interestRatePerSecond + BASE,
            duration,
            BASE
        ) / BASE;
    }

    function getIntrestRateForDuration(uint256 interestRatePerSecond, uint256 duration) internal pure returns (uint256) {
        return rpow(
            interestRatePerSecond + BASE,
            duration,
            BASE
